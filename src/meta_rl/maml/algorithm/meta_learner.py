...
class MetaLearner:
    ...
    def collect_train_data(self, indices: np.ndarray, is_eval: bool = False) -> None:
        ...
        for cur_task, task_index in enumerate(tqdm(indices)):  # TODO meta_batch_size 번 반복
            ...
            for cur_adapt in range(self.num_adapt_epochs + 1):
                # TODO num_adapt_epochs 번 적응 후 1 번 테스트

                # 메타-테스트에 대해서는 deterministic한 정책으로 경로 생성
                self.agent.policy.is_deterministic = (
                        cur_adapt == self.num_adapt_epochs and is_eval  # TODO 적응 이후 마지막 시도에 test
                )
                ...
                if cur_adapt < self.num_adapt_epochs:  # TODO num_adapt_epochs 번 적응
                    ...
                    # before_weight = dict(self.agent.policy.named_parameters())['fc_layers.1.weight']
                    if require_grad:
                        print("require_grad is True")
                    inner_loss.backward(create_graph=require_grad)

                    # intermediate_weight = dict(self.agent.policy.named_parameters())['fc_layers.1.weight']
                    with torch.set_grad_enabled(require_grad):  # TODO requrie_grad = False여도 optim.step() 먹힘
                        self.inner_optimizer.step()
                    # after_weight = dict(self.agent.policy.named_parameters())['fc_layers.1.weight']

                    # print()
                    # print(before_weight[0][0:3])
                    # print(intermediate_weight[0][0:3])
                    # print(after_weight[0][0:3])
                    # print(before_weight[0][0:3] == intermediate_weight[0][0:3])
                    # print(before_weight[0][0:3] == after_weight[0][0:3])
    ...
    def meta_train(self) -> None:
        ...
        for iteration in range(self.num_iterations):
            ...
            # indices = [0 ~ train_tasks) * meta_batch_size
